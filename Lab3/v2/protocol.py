...
PACKET_SPAWN_ISLAND = 0x13
PACKET_SPAWN_WHIRLPOOL = 0x14
...
class GameStatePacket:
    ...
    def __init__(self):
        ...
        self.player_y = 0
        self.player_hp = 100
        self.player_angle = 0  # Угол корабля
        self.score = 0
        self.level = 1
    ...
    def parse(data):
        ...
        idx += 1
        packet.player_angle = struct.unpack('b', data[idx:idx+1])[0]  # Знаковый байт
        idx += 1
...
class SpawnIslandPacket:
    """Пакет спавна острова PC -> STM32"""
    ...
    def encode(self):
        ...
        data = bytearray()
        data.append(START_BYTE)
        data.append(PACKET_SPAWN_ISLAND)
        data.extend(struct.pack('>H', self.x))
        data.extend(struct.pack('>H', self.y))
        data.append(self.size)
        data.append(crc8(data[1:]))
        data.append(END_BYTE)
        return bytes(data)

class SpawnWhirlpoolPacket:
    """Пакет спавна водоворота PC -> STM32"""
    ...
    def encode(self):
        ...
        data = bytearray()
        data.append(START_BYTE)
        data.append(PACKET_SPAWN_WHIRLPOOL)
        data.extend(struct.pack('>H', self.x))
        data.extend(struct.pack('>H', self.y))
        data.extend(struct.pack('>H', self.target_x))
        data.extend(struct.pack('>H', self.target_y))
        data.append(crc8(data[1:]))
        data.append(END_BYTE)
        return bytes(data)
