...
/**
  * @file           : main.c
  * @brief          : Endless Sea - Battle Ship on STM32
  */
...
int main(void)
{
  ...
  while (1)
  {
      ...
      if (current_tick - last_tick >= FRAME_TIME_MS) {
          ...
          // 1. Обработка входящих данных от PC
          Protocol_ProcessIncoming(&game);
          
          // 2. Обработка ввода с кнопок
          Input_Update(&input);
          
          // 3. Стрельба при нажатии кнопки
          ...
          // 5. Отправка состояния игры на PC
...
// ????????? ??????? ???????? ??? ?????????...
// (SystemClock_Config, MX_USART2_UART_Init, MX_DMA_Init, MX_GPIO_Init)
