...
class Game:
    ...
    def _draw_waves(self):
        ...
        AMPLITUDE = 12        # Высота волны (амплитуда)
        WAVE_LENGTH = 80      # Длина волны (расстояние между пиками)
        WAVE_SPEED = 0.03     # Скорость движения волн
        VERTICAL_SPACING = 35 # Вертикальное расстояние между слоями волн
        
        # Основное смещение для анимации волн
        base_offset = (self.camera_y // 3) % VERTICAL_SPACING + (self.wave_offset % VERTICAL_SPACING)
        
        # Рисуем несколько слоев волн для объемного эффекта
        for layer in range(-2, SCREEN_HEIGHT // VERTICAL_SPACING + 3):
            # Вертикальная позиция текущего слоя
            base_y = layer * VERTICAL_SPACING + base_offset
            
            # Плавное изменение цвета в зависимости от глубины
            ...
            # Фаза волны (смещение для создания эффекта движения)
            phase_shift = layer * 0.8  # Разное смещение для каждого слоя
            points = []
            
            # Генерируем точки волны с синусоидальными колебаниями
            for x in range(0, SCREEN_WIDTH + WAVE_LENGTH, 5):
                ...
                # Добавляем вторую гармонику для реалистичности
            ...
            # Рисуем волну с плавными изгибами
            if len(points) > 1:
